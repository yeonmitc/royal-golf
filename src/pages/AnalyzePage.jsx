...
export default function AnalyzePage() {
  ...
  async function load(nextFrom = fromDate, nextTo = toDate) {
    if (typeof nextFrom !== 'string') nextFrom = fromDate;
    if (typeof nextTo !== 'string') nextTo = toDate;
    setPending(true);
  ...
  const isStale =
    Boolean(data) &&
    hasApplied &&
    (String(appliedRange?.from || '') !== String(fromDate || '') ||
      String(appliedRange?.to || '') !== String(toDate || ''));
  const titleFrom = data && hasApplied ? appliedRange.from : fromDate;
  const titleTo = data && hasApplied ? appliedRange.to : toDate;
  ...
  return (
    <div className="page-root">
      ...
      {isStale ? (
        <div className="text-sm text-[var(--text-muted)]" style={{ marginBottom: 10 }}>
          Date range changed. Click Analyze to refresh results.
        </div>
      ) : null}

      <Card title={`Sales Summary (${titleFrom} ~ ${titleTo})`}>
        {!data ? (
          ...
          >
            ...
            <div className="page-card">
              <div>SoldAt Range</div>
              <div style={{ fontWeight: 700 }}>
                {String(data.summary.sourceSoldAtMin || data.summary.soldAtMin || '-').slice(0, 16)}
              </div>
              <div className="muted" style={{ marginTop: 2 }}>
                {String(data.summary.sourceSoldAtMax || data.summary.soldAtMax || '-').slice(0, 16)}
              </div>
            </div>
          </div>
        )}
